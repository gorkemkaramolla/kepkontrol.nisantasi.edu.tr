        ...
        if (userInformation) {
            ...
            fetch(`/api/etkinlikler/${student_number}/`)
                ...
                .then(data => {
                    ...
                    const eksik_ayni_fakulte = data.filter(item => item.KATILIM === "" && item.AYNI_FAKULTE ===
                        "Fakülte/MYO içi").length;;
                    const eksik_farkli_fakulte = data.filter(item => item.KATILIM === "" && item.AYNI_FAKULTE ===
                        "Fakülte/MYO dışı").length;
